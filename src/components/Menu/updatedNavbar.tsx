...
export default function Navbar() {
  ...
  return (
    <main className={`flex w-20 py-6 justify-between items-center flex-col h-screen bg-white ${isRtl ? 'rtl' : 'ltr'}`}>
      <div>
        <Image 
            src={'/menu/rightarrow.png'}
            height={15}
            width={15}
            alt=''
          />
      </div>
      <NavigationMenu className={`relative flex   px-4 py-4 ${isRtl ? 'flex-row-reverse' : ''}`}>
        <NavigationMenuList className={`flex flex-col gap-4 ${isRtl ? 'flex-row-reverse' : ''}`}>
...
function DropdownMenuItem({ iconSrc, dropdownData }: DropdownMenuItemProps) {
  ...
  return (
    ...
    >
      <img src={iconSrc} alt="Icon" className="w-8 h-8 cursor-pointer" />

      {isDropdownVisible && (
        <div 
          className={`absolute w-72 top-0 left-full ml-8 bg-white z-100 text-black shadow-lg p-4 ${isRtl ? 'right-full ml-0 mr-4' : 'left-full ml-4'}`}
        >
